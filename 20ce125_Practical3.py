...
# To store total number of families into NewK variable.
NewK = (len(MyList1)-1) // K
...
# Loop to find which element occures only one time 
j = 0
while j < len(MyList2) :
    if (MyList1.count(MyList2[j])) == 1:
        Answer = MyList2[j]
    elif (MyList1.count(MyList2[j])) == K:
        count += 1
    j += 1

# If Total Families are equal to NewK variable then Answer will be printed otherwise it will print error message.
print(f"\nOutput:")
if count == NewK:
    print(f"{Answer}\n")
else:
